...
WEB_PORT = int(os.environ.get("PANEL_WEB_PORT", "8088").strip() or "8088")
...
def force_sync_repo_current_branch(repo_dir: str) -> str:
    ...
    # 修复：直接使用原始分支名，禁止拼接带引号的safe_branch
    return run(
        "set -e; "
        f"cd {safe_dir} && "
        f'echo "[deploy] CURRENT_BRANCH={branch}" 2>&1 && '
        "git fetch origin --prune 2>&1 && "
...
def switch_web_branch(branch: str) -> str:
    ...
    # 优化：添加set -e + ff-only，避免冲突合并
    return run(
        "set -e; "
        f"cd {safe_dir} && "
        f"git checkout {safe_branch} 2>&1 && "
        f"git pull --ff-only origin {safe_branch} 2>&1"
    )
...
def detect_action_kind(text: str) -> str:
    ...
    if any(token in lowered for token in
           ["files changed", "changed", "updating", "fast-forward", "switched to branch"]):
        return "success"
    return "muted"
