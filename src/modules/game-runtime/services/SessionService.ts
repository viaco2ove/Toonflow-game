...
async function applySessionUserEventProgress(params: {
  ...
  // ★ AI #2 分流（事件进度检测）
  // - user phase → 规则标记完成，不调 AI
  // - scene/fixed/opening 等非 user phase + 非 "." 快路径 + forceAi=false → 跳过 AI，事件不推进
  // - scene 等阶段 + "." 快路径 或 forceAi=true → 保留 AI #2，判定事件是否推进
  {
    ...
    if (isUserPhase) {
      ...
      console.log("[applySessionUserEventProgress] user phase，跳过事件进度 AI，走规则标记完成", {
        phaseId: currentProgress.phaseId,
        phaseKind: currentPhase?.kind || "unknown",
      });
    ...
    console.log("[applySessionUserEventProgress] 非 user phase，调用事件进度 AI（'.' 或 forceAi）", {
      phaseId: currentProgress.phaseId,
      phaseKind: currentPhase?.kind || "unknown",
      forceAi: params.forceAi,
    });
...
async function addSessionMessageInner(input: AddSessionMessageInput, sessionId: string): Promise<AddSessionMessageResult> {
  ...
  if (currentChapter) {
    if (roleTypeValue === "player" && eventTypeValue === "on_message" && messageContent.trim()) {
      ...
      recentMessagesForProgress = buildRecentMessages(rawRecentMessagesForProgress, state);

      if (messageContent.trim() === ".") {
        const msgs = rawRecentMessagesForProgress;
        // 找最近一条玩家消息（排除当前这条，因为它还没入库）
        const prevPlayerMsg = msgs.find((m: any) => String(m.roleType || "").trim().toLowerCase() === "player");
        if (prevPlayerMsg && String(prevPlayerMsg.content || "").trim() === ".") {
          // 检查该 "." 之后是否有 NPC/旁白
          const prevIdx = msgs.indexOf(prevPlayerMsg);
          const afterPrev = msgs.slice(prevIdx + 1);
          const hasNpcOrNarrator = afterPrev.some((m: any) => {
            const rt = String(m.roleType || "").trim().toLowerCase();
            return rt === "narrator" || rt === "role" || rt === "npc";
          });
          if (!hasNpcOrNarrator) {
            DebugLogUtil.log("story:ai_parallel", "[addSessionMessage] 兜底: 检测到连续 '.'（前一次无 NPC 响应），强制走 AI 完整链路");
            forceAi = true;
          }
        }
      }
  ...
  if (currentChapter) {
    // ★ AI #2 ∥ AI #3 并发优化
    //    applySessionUserEventProgress（内含 AI #2）和 evaluateRuntimeOutcome（内含 AI #3）
    //    两者输入基本独立（都读当前 state 快照），可以并发。
    //    总耗时 ≈ max(AI #2, AI #3) 而非相加，省 3-5s。
...
async function orchestrateSessionTurnInner(sessionId: string): Promise<SessionOrchestrationResult> {
  ...
  const applyWorldClockForTurn = (narrativeProducedThisTurn: boolean, resultPlan: any, chapterForClock: any): void => {
    ...
    if (timeMode === "realtime") {
      ...
      if (jump === 0) {
        ...
        const explicitTick = typeof explicit?.tick === "number" && Number.isFinite(explicit.tick) ? explicit.tick : -1;
        ...
        if (explicitTick !== -1) {
          ...
          DebugLogUtil.log("story:orchestrator:runtime", "[worldClock] narrative trigger-1(explicit)", JSON.stringify({
            explicitTick,
            explicitWeather,
            jump,
            weatherOverride,
  ...
  if (mergedOutcome.outcome === "success" || ( nextStatus ==="chapter_completed")) {
    ...
    DebugLogUtil.log("story:orchestrator:chapter_switch"," 编排结果章节判定 outcome resolvedNextChapterId being");
    let resolvedNextChapterId =realNextChapterId;
    try {
    ...
    DebugLogUtil.log("story:orchestrator:chapter_switch"," try catched resolvedNextChapterId");
  ...
  const eventView = buildEventView(state);
  DebugLogUtil.log("story:orchestrator:chapter_switch"," SessionOrchestrationResult chapterId:", {
     chapterId: nextChapterId,
     oldChapterId:currentChapterId,
    sesionChapterId:sesionChapterId,
    sessionStatus:realSessionStatus,
