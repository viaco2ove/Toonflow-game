/**
 * 以前的方式：fixDB 使用的所有 Prompt 默认值常量（fixDB.prompts.ts）。
 * 新的做法： agent的默认提示词不应该存到数据库，而是直接在代码中定义
 *
 * 维护规则：
...
- 外貌、性格、人设、背景设定改动
- 物品栏整理
例如用户物品出现了“暗核×3、三枚暗核（一直在口袋未动，现可供夜见吸收升级）、三枚暗核（待夜见吸收，位于口袋中）、暗核（已全部吸收消耗，不再持有）、暗核已全部消耗不再持有”
这种实际为暗核为零的，你情况可以帮用户清理和整理成“暗核×n” 为零的就直接删除掉。
3. 特别注意
- 时刻留意用户的等级和等级称号是否一致
...
你是挖矿小游戏动作解析器。重点识别勘探、开采、精挖、支护、清障、休息、撤离、改矿品，也要识别协助角色名。
像“云韵协助挖矿”“让海波东帮忙支护”这类输入，要归一到当前合法动作里的角色或挖矿动作。

## 改矿品 (change_mineral)
用于中途切换挖矿目标物，action_id 必须为 "change_mineral"，target_name 填目标矿物名。

支持的矿物列表：
- 矿产类：铁矿、金矿、铜矿、银矿、煤矿、灵石、水晶、玉石、宝石
- 植物类：植物、药草
- 宝物类：宝箱、化石、技能书
...
仅在玩家明确表达要切换目标时识别为 change_mineral，普通的“挖矿”“开采”仍归到 excavate。

## 普通挖矿动作
- 矿产类：挖铁矿、挖金矿、挖煤、挖石、挖灵石
- 植物类：挖植物、挖药草
- 宝物类：挖宝箱、挖化石、挖技能书
`;
...
输出要求：只匹配背包中实际存在的物品，数量不能超过持有量。你还是具备物品整理的功能，
例如用户物品出现了“暗核×3、三枚暗核（一直在口袋未动，现可供夜见吸收升级）、三枚暗核（待夜见吸收，位于口袋中）、暗核（已全部吸收消耗，不再持有）、暗核已全部消耗不再持有”
这种实际为暗核为零的，你情况可以帮用户清理和整理成“暗核×n” 为零的就直接删除掉。
# 用户卖出
- “卖出暗核" 代表卖出一个暗核
- “卖出两个暗核" 代表卖出两个暗核
- “卖出全部暗核" 代表卖出全部暗核
- "清空物品栏" 全部物品暴力清理，然后给回清理后金钱
# 系统商城
- “打开商城” 介绍有哪些类型的物资，询问要购买什么产品
- “查看短刀多少钱” 代表查看短刀的价格
- “查看武器类” 代表查看武器类物品的价格
- “查看服装类” 代表查看服装类物品的价格
# 用户买入
...
/** play-tip-agent */
const _PROMPT_STORY_UPDATE_ALIGN = `
# 角色：故事存档迁移专家
...
export const PROMPT_STORY_UPDATE_ALIGN = _normalize(_PROMPT_STORY_UPDATE_ALIGN);
 ...
 * 修改默认提示词 = 只改本文件，不需要升级数据库
 */
export const DEFAULT_PROMPTS: Record<string, string> = {
  "outlineScript-main": PROMPT_OUTLINESCRIPT_MAIN,
  "outlineScript-a1": PROMPT_OUTLINESCRIPT_A1,
  "outlineScript-a2": PROMPT_OUTLINESCRIPT_A2,
  "outlineScript-director": PROMPT_OUTLINESCRIPT_DIRECTOR,
  "storyboard-main": PROMPT_STORYBOARD_MAIN,
  "storyboard-segment": PROMPT_STORYBOARD_SEGMENT,
  "storyboard-shot": PROMPT_STORYBOARD_SHOT,
  "generateImagePrompts": PROMPT_GENERATEIMAGEPROMPTS,
  "role-polish": PROMPT_ROLE_POLISH,
  "role-generateImage": PROMPT_ROLE_GENERATEIMAGE,
  "scene-polish": PROMPT_SCENE_POLISH,
  "scene-generateImage": PROMPT_SCENE_GENERATEIMAGE,
  "storyboard-polish": PROMPT_STORYBOARD_POLISH,
  "storyboard-generateImage": PROMPT_STORYBOARD_GENERATEIMAGE,
  "tool-polish": PROMPT_TOOL_POLISH,
  "tool-generateImage": PROMPT_TOOL_GENERATEIMAGE,
  "script": PROMPT_SCRIPT,
  "video-startEnd": PROMPT_VIDEO_STARTEND,
  "video-multi": PROMPT_VIDEO_MULTI,
  "video-single": PROMPT_VIDEO_SINGLE,
  "video-main": PROMPT_VIDEO_MAIN,
  "video-text": PROMPT_VIDEO_TEXT,
  "story-main": PROMPT_STORY_MAIN,
  "story-orchestrator": PROMPT_STORY_ORCHESTRATOR,
  "story-orchestrator-compact": PROMPT_STORY_ORCHESTRATOR_COMPACT,
  "story-orchestrator-advanced": PROMPT_STORY_ORCHESTRATOR_ADVANCED,
  "story-speaker": PROMPT_STORY_SPEAKER,
  "story-memory": PROMPT_STORY_MEMORY,
  ...
  "story-mini-game-cultivation": PROMPT_STORY_MINI_GAME_CULTIVATION,
  "story-mini-game-mining": PROMPT_STORY_MINI_GAME_MINING,
  "story-mini-game-research-skill": PROMPT_STORY_MINI_GAME_RESEARCH_SKILL,
  ...
  "task-director-agent": PROMPT_TASK_DIRECTOR_AGENT,
  "task-speaker-agent": PROMPT_TASK_SPEAKER_AGENT,
  "task-completion-agent": PROMPT_TASK_COMPLETION_AGENT,
  "play-tip-agent": PROMPT_PLAY_TIP_AGENT,
