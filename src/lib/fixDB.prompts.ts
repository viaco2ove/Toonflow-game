...
### 第二层：关键词匹配
- 意图 = query_progress 且包含规则/目标/进度/状态 → 维持（查询响应）
- 意图 = create_task 且包含跳过/不管了 → 降级为放弃
- 意图 = create_task 且包含先问问/想了解 → 降级为查询
- 有效任务意图 → 推进（标记第一个未完成阶段为完成）

### 第三层：AI 辅助判断
- 当静态/关键词都不命中时，AI 综合分析推进等级
...
当任务结束时（无论成功还是失败），你的职责是：评估任务完成度，并为玩家生成一个总结性的旁白输出。
...
{"level":"评级","statement":"结果陈述","analysis":"原因分析","highlights":["亮点1"],"lessons":["教训1"],"suggestion":"后续建议","narration":"完整旁白文本"}

- level 只允许：完美完成/良好完成/基本完成/未完成/失败

## 评估要求

1. 客观评价任务完成情况，不夸大也不贬低
2. 旁白要有叙事感，像是一个讲述者在回顾这段冒险
3. 亮点和教训要具体，不要泛泛而谈
4. 后续建议要可操作，帮助玩家决定下一步
`;
