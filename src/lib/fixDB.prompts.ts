...
玩家每次有效输入后，AI 要判断如何更新任务推进过程。

### 过程格式
每个阶段格式为：[状态]描述文本
状态标记：[]空闲 [i]进行中 [s]完成 [f]失败

### 更新动作
- mark_complete：玩家完成了当前阶段，标记为 [s]
- mark_failed：玩家该阶段遇到困难，标记为 [f]（不直接判任务失败，可插入新阶段重试）
- insert：玩家行为需要新阶段，插入到当前阶段之后
- none：不更新推进过程

### 更新策略
- 玩家行为有效推进 → mark_complete（标记已完成阶段）
- 玩家行为失败/遇阻 → mark_failed + 可选 insert（插入应对阶段）
- 玩家查询状态 → none
- 玩家提供关键信息 → mark_complete + 可选 insert（自动生成下一阶段）
- 所有阶段都 [s] → 任务完成，触发 Completion Agent

## 输出格式（严格 JSON）

{\"level\":\"等级\",\"tier\":\"判定层级\",\"reason\":\"理由\",\"needClarify\":true/false,\"clarifyContent\":\"追问内容\",\"processUpdate\":{\"action\":\"none|mark_complete|mark_failed|insert\",\"phaseIndex\":数字|null,\"newPhase\":\"新阶段文本(仅insert时)\"}}

- level 只允许：强力推进/正常推进/微弱推进/维持/放弃
- tier 只允许：静态条件/关键词匹配/AI辅助判断
- processUpdate.action 只允许：none/mark_complete/mark_failed/insert
- needClarify=true 时填写 clarifyContent
`;
