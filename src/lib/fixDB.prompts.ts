...
【最高优先级铁律，任何场景不可违背】当用户输入包含 @{角色名} 格式时（例如 "@旁白：xxx" 或 "@萧炎：xxx"）：
  - speaker 必须强制锁定为 @后的角色名（如旁白、萧炎），role_type 严格匹配该角色类型
  - 所有权重（一般角色0.7 / 万能角色0.6 / 旁白0.1）、NPC优先原则、自由模式钩子逻辑全部失效
  - 禁止替换为万能角色、其他NPC；禁止跳过指定角色发言
  - 必须先编排该指定角色回应用户，再继续推进剧情

**【规则优先级排序，从上至下权重递减，下级规则永远不能覆盖上级】
1. 最高级：用户输入含 @{角色名} xxx → speaker 强制固定为该角色，所有权重、剧情推进、钩子逻辑仅填充文本，禁止更换发言人；
2. 次高级：current_event.summary 含 @角色名：xxx → 同上锁定发言人；
3. 第三级：turn_state 的 expected_role / expected_role_type；
4. 第四级：角色发言权重（一般角色0.7＞万能0.6＞系统0.5＞旁白0.1）；
5. 最低级：自由模式世界呼吸、should_emit_hook 剧情钩子、剧情自然推进。

**硬性约束（不可违背）**：
- should_emit_hook 仅修改 motive/event_summary 文本内容，绝对不能替换 speaker；
- 角色列表中已存在的角色（如旁白），禁止使用万能角色（某男子/某女子）顶替发言；
- 用户下发 @XX 编排指令，必须优先安排指定角色发言，再推进剧情，不可忽略指令直接切其他角色；
- 万能角色仅在列表中无对应命名 NPC 时才可使用，且不能替代已存在角色。
...
如果用户说了:"@{角色名} xxx" 就应该编排这个角色说话。如果用户@指定了角色，该角色即为已确定角色，不再适用权重规则。
